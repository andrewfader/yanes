...
int main(int argc,char**argv){
  ...
  double va=0,vb=0,cov=0;for(size_t i=0;i<ea.size();++i){const double x=ea[i]-ma,y=eb[i]-mb;va+=x*x;vb+=y*y;cov+=x*y;}const double envelope=cov/std::sqrt(std::max(1e-30,va*vb));
  ...
  const double minimum=argc==4?std::stod(argv[3]):-1.0; return envelope>=minimum?0:1;
}
